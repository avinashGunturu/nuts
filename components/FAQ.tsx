            ...
            <div className="space-y-3">
              {FAQS.map((faq, index) => (
                ...
                >
                  ...
                  >
                    <span className={`text-base md:text-lg font-bold transition-colors ${openIndex === index ? 'text-brand' : 'text-neutral-900'}`}>
                      {faq.question}
                    </span>
                    <div className={`flex-shrink-0 ml-3 w-8 h-8 md:w-10 md:h-10 rounded-full flex items-center justify-center transition-all duration-300 ${openIndex === index ? 'bg-brand text-white rotate-180' : 'bg-neutral-100 text-neutral-500 hover:bg-neutral-200'}`}>
