...
export const Header: React.FC<HeaderProps> = ({ cartCount }) => {
  ...
  return (
    <>
      ...
      {/* Mobile Menu - Now OUTSIDE header for proper z-index stacking */}
      {isMobileMenuOpen && (
        <div className="fixed inset-0 bg-white z-[60] flex flex-col pt-24 px-8 animate-fade-in md:hidden">
          {/* Header spacer with logo area */}
          <div className="absolute top-0 left-0 right-0 h-20 bg-white border-b border-neutral-100 flex items-center justify-between px-6">
            <Link to="/" className="flex items-center gap-3" onClick={() => setIsMobileMenuOpen(false)}>
              <div className="w-10 h-10 rounded-xl flex items-center justify-center font-bold bg-brand text-white text-xl shadow-lg shadow-brand/20">KC</div>
          ...
          <nav className="flex flex-col gap-1 mt-2">
            {[
              ...
              <Link
                ...
                to={item.path}
                onClick={() => setIsMobileMenuOpen(false)}
                className="text-xl font-bold text-neutral-900 tracking-tight py-2 px-4 rounded-xl hover:bg-neutral-50 transition-colors"
              >
                {item.name}
              </Link>
            ))}
            <div className="pt-3 mt-3 border-t border-neutral-100 flex flex-col gap-1">
              {isAuthenticated ? (
                <>
                  <Link to="/orders" onClick={() => setIsMobileMenuOpen(false)} className="text-xl font-bold text-brand tracking-tight py-3 px-4 rounded-xl hover:bg-brand-50 transition-colors">Your Orders</Link>
                  <Link to="/profile" onClick={() => setIsMobileMenuOpen(false)} className="text-xl font-bold text-brand tracking-tight py-3 px-4 rounded-xl hover:bg-brand-50 transition-colors">Profile Settings</Link>
                </>
              ) : (
                <Link to="/login" onClick={() => setIsMobileMenuOpen(false)} className="text-xl font-bold text-brand tracking-tight py-3 px-4 rounded-xl hover:bg-brand-50 transition-colors">Sign In / Join</Link>
              )}
            </div>
          </nav>
          <div className="mt-auto mb-8 pt-6">
            <button onClick={() => { setIsMobileMenuOpen(false); openCart(); }} className="flex items-center justify-between w-full bg-brand text-white p-5 rounded-2xl shadow-xl shadow-brand/20">
